...
void printFIX(int32_t val){
    if ((val & 0x80000000) != 0) { // Handle negative numbers
        printf("-");
        val = ~val + 1;
    }
    printf("%ld.%04ld\n", val >> 14, 10000 * (uint32_t)(val & 0xFFFF) >> 14);
    // Print a maximum of 4 decimal digits to avoid overflow
}

int32_t calcSINUS(int32_t angle){
    angle = angle % 360;
    int32_t newAngle = FIX14_MULT(FIX14_DIV(angle, 360),512);
    int32_t sinAngle = SIN[newAngle];
    return sinAngle;
}

int32_t calcCOSINUS(int32_t angle){
    angle = angle + 90;
    int32_t cosAngle = calcSINUS(angle);
    return cosAngle;
}
